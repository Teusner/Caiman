...
with st.sidebar:
    ...
    if controls[0].button("Step ×1", use_container_width=True, disabled=not sim.deployed or terminal):
        sim.step(force=True)
        st.session_state.skip_auto_step_once = True
...
refresh_sim = st.session_state.simulation
refresh_interval = max(0.25, 1.0 / refresh_sim.config.simulation_speed) if refresh_sim.running else None
...
def render_dashboard() -> None:
    ...
    skip_auto_step = st.session_state.pop("skip_auto_step_once", False)
    if sim.running and not skip_auto_step:
    ...
    if was_running and not sim.running:
        st.rerun()
